...
namespace CAPA_DATOS
{
    public class CD_Consulta_consul
    {
        ...
        public List<Modelo> ObtenerREgistros_porMaterias(int ID_materias)
        {
            ...
            {
                comando.Connection = conexion.AbrirConexion();//conexion al servidor
                comando.CommandText = "SELECT * FROM  Consultas WHERE IDmaterias="+ID_materias;//consulta
                leer = comando.ExecuteReader();//llemos los datos encontrados
                                               //mientras existan registros por leer cargamos los datos en la cola
                while (leer.Read())
                {
        ...
        public List<Modelo> ObtenerREgistros_porSemana(int NumeroSemana)
        {
            ...
            {
                comando.Connection = conexion.AbrirConexion();//conexion al servidor
                comando.CommandText = "SELECT * FROM  Consultas WHERE Numero_semana=" + NumeroSemana;//consulta
                leer = comando.ExecuteReader();//llemos los datos encontrados
                                               //mientras existan registros por leer cargamos los datos en la cola
                while (leer.Read())
                {
        ...
        public List<Modelo> ObtenerREgistros_porCarnet(string Carnet)
        {
            ...
            {
                comando.Connection = conexion.AbrirConexion();//conexion al servidor
                comando.CommandText = "SELECT * FROM  Consultas WHERE N_Carnet='"+Carnet+"'";//consulta
                leer = comando.ExecuteReader();//llemos los datos encontrados
                                               //mientras existan registros por leer cargamos los datos en la cola
                while (leer.Read())
                {
        ...
        //----------------------------------------------------------------------------------------------------------------------------------------
        public void EditarConsulta(int ID_materias, int Numero_semana, int Cantidad_Estudiante,
            string Tipo_consulta, string Tema_consulta, DateTime Fecha, string Hora, string Numero_Carnet, string Firma, string Observaciones, int VoBo_Jefe)
        {
            //validar exception

            comando.Connection = conexion.AbrirConexion();//nos conectamos al servidor
            string consulta = "Update Consultaa SET" + "Numero_semana=" + Numero_semana + ",Cantidad_estudiante=" +
                    Cantidad_Estudiante + ",Tipo_consulta='" + Tipo_consulta + "',Tema_consulta='" + Tema_consulta + "'Hora='" + Hora + "',N_Carnet='" + Numero_Carnet + "',Firma='" + Firma + "',Observaciones='" + Observaciones + "',Vobo_JefeDept=" + VoBo_Jefe + ")";
            comando.CommandText = consulta;//pasamos la consulta
                                           //ejecutamos la consulta
            comando.ExecuteNonQuery();
            //cerramos la conexion
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------
        public void Editar_Consult(int ID_CONSULT)
        {
            comando.Connection = conexion.AbrirConexion();
            string Consulta = "UPDATE Consultas SET  WHERE ID_Consultas= " + ID_CONSULT;//consulta sql
            comando.CommandText = Consulta;//pasamos la consulta
            comando.ExecuteNonQuery();
            conexion.CerrarConexion();
        }
